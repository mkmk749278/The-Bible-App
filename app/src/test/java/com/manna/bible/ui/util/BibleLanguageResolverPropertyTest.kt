 ...
 * [GraphicsMode] NATIVE, and a **bounded** `waitUntil` keyed on an emitted marker node rather
 * than an unbounded `waitForIdle`. The composition emits a real marker [Text] (`"seen_<id>"`)
 * so each state change actually produces a frame the bounded `waitUntil` can observe — an
 * empty composition produces no frames, so a content-free recompose signal would never settle.
 */
    ...
    fun `resolving with a blank tag matches resolving against the UI locale`() {
        ...
        runComposeUiTest {
            ...
            setContent {
                ...
                // A real emitted node makes each recompose produce a frame and gives the
                // bounded waitUntil below a rendered signal to key on (per id).
                MannaTheme { Text("seen_$id") }
            }
            runBlocking {
                checkAll(PropTestConfig(iterations = 20), Arb.of(ids)) { id ->
                    idState.value = id
                    waitUntil(timeoutMillis = 5_000) {
                        onAllNodesWithText("seen_$id").fetchSemanticsNodes().isNotEmpty()
                    }
                    assertEquals(
