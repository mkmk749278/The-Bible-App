...
- **Configuration (cost-split iteration budget):** iteration counts are split by property cost. This refines only execution parameters — it changes no requirement and no correctness property.
  - **Pure-domain / JVM property tests** — Properties 1, 2, 6, 7, 11, 14, 15, 16, 17, 18 (serialization round-trip, parse order, language/content consistency, `LocalizedText` fallback determinism, role-color distinctness, denomination mapping totality/fallback, placeholder preservation, gap-check correctness, blank-tag resolution), plus the JVM manifest properties 4 and 5 — run at a **minimum of 100 iterations** (`PropTestConfig(iterations = 100)`, or higher for cheap pure-function properties). These are the combinatorially meaningful invariants and stay at full strength.
  - **Rendering property tests** that require Robolectric + Compose — Properties 3 (library-exclusion rendering side), 8, 9, 10, 12, 13 (source note rendered, rendered content resolved, role-label totality in the UI, official-text notice shown, scripture openable) — run at a **reduced minimum of 20 iterations**. Rationale: each Robolectric iteration boots a full Android runtime and renders a Compose tree, so runtime grows roughly linearly and 100+ iterations push the suite past ~10 minutes; the rendered output varies far less per iteration than pure-domain inputs, so 20 iterations preserve strong coverage at a fraction of the cost.
...
- **Mapping** of the 18 properties to tests (one property-based test each): Properties 1–18 above. Pure-domain properties (1, 2, 6, 7, 11, 14, 15, 16, 17, 18) run on the JVM at ≥100 iterations; rendering properties (3 library-exclusion side, 8, 9, 10, 12, 13) run under Robolectric/Compose with generated liturgies at the reduced 20-iteration budget; manifest properties (4, 5) run against a generated manifest + in-memory asset map on the JVM at ≥100 iterations.
- **Execution strategy:** during development, run targeted subsets via Gradle `--tests` filters for the new test classes; reserve a single full `testDebugUnitTest` run for the final integration/verification step. The property-test tag format (`// Feature: mass-liturgy-and-localization, Property {number}: {property_text}`) and the set of properties covered are unchanged.
